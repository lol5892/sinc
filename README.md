...
**Telegram Mini App** — совместное планирование недели (слева шкала 0–24, сверху дни; в клетках есть плюс для добавления дела; блоки дел перетаскиваются и тянутся по высоте) + бот с напоминаниями в чат. Стек: Vite + React, Express, данные в файле `data/events.json`, Telegraf.
...
4. Оба пользователя нажали **Start** у бота — иначе личные напоминания могут не доставиться. Команда `/plan` повторно показывает кнопку открытия планера.
